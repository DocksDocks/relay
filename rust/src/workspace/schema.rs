...
impl RecoverRequestV1{
 pub fn validate(&self)->Result<(),String>{
  ...
  if self.expected_state==WorkspaceState::IntegrationBlocked&&!matches!(self.action.as_str(),"inspect"|"retain_abort"){return Err("IntegrationBlocked permits inspect or explicit retain_abort only; integration is never retried".into())}
  ...
  if self.action=="retain_abort"&&!matches!(self.expected_state,WorkspaceState::Running|WorkspaceState::HandbackReady|WorkspaceState::IntegrationBlocked){return Err("retain_abort requires Running, HandbackReady, or IntegrationBlocked".into())}
...
impl FinishRequestV1{
 pub fn validate(&self)->Result<(),String>{validate_coordinator_cas(&self.repository_path,&self.repository_id,&self.request_id,&self.session_id,&self.expected_journal_head_sha256,&self.expected_head,&self.created_at)?;if self.expected_state==WorkspaceState::IntegrationBlocked{return if self.acknowledge_needs_user_action{Ok(())}else{Err("IntegrationBlocked finish requires acknowledge_needs_user_action=true".into())}}if !matches!(self.expected_state,WorkspaceState::Integrated|WorkspaceState::Rejected|WorkspaceState::AbortedRetained){return Err("finish expected_state must be Integrated, Rejected, AbortedRetained, or acknowledged IntegrationBlocked".into())}if self.acknowledge_needs_user_action{return Err("acknowledge_needs_user_action is invalid outside IntegrationBlocked".into())}Ok(())}
}
...
mod tests {
 ...
 #[test] fn integration_blocked_requires_explicit_retained_abort_acknowledgement(){
  let request_id="11111111-1111-4111-8111-111111111111".to_string();let session_id="22222222-2222-4222-8222-222222222222".to_string();let oid="a".repeat(40);let digest="c".repeat(64);let at="2026-07-22T12:34:56.789Z".to_string();
  assert!(WorkspaceState::IntegrationBlocked.may_transition_to(WorkspaceState::AbortedRetained));assert!(!WorkspaceState::IntegrationBlocked.may_transition_to(WorkspaceState::IntegrationQueued));assert!(!WorkspaceState::IntegrationBlocked.may_transition_to(WorkspaceState::Running));
  let finish=|acknowledge_needs_user_action|FinishRequestV1{request_id:request_id.clone(),repository_path:"/repo".into(),repository_id:digest.clone(),session_id:session_id.clone(),expected_state:WorkspaceState::IntegrationBlocked,expected_journal_head_sha256:digest.clone(),expected_head:oid.clone(),acknowledge_needs_user_action,created_at:at.clone()};assert!(finish(false).validate().is_err());assert!(finish(true).validate().is_ok());
  assert!(AbortRequestV1{request_id:request_id.clone(),repository_path:"/repo".into(),repository_id:digest.clone(),session_id:session_id.clone(),expected_state:WorkspaceState::IntegrationBlocked,expected_journal_head_sha256:digest.clone(),expected_head:oid.clone(),reason:"explicit retained abort after conflict".into(),created_at:at.clone()}.validate().is_ok());
  let recover=|action:&str|RecoverRequestV1{request_id:request_id.clone(),repository_path:"/repo".into(),repository_id:digest.clone(),session_id:session_id.clone(),expected_state:WorkspaceState::IntegrationBlocked,expected_journal_head_sha256:digest.clone(),expected_head:oid.clone(),action:action.into(),created_at:at.clone()};assert!(recover("inspect").validate().is_ok());assert!(recover("retain_abort").validate().is_ok());assert!(recover("resume_prelaunch").validate().is_err());
 }
}
