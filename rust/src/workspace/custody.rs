...
impl ControlPacket {
    ...
    pub fn decode(bytes: &[u8], key: &[u8; 32]) -> Result<Self, String> {
        if bytes.is_empty() || bytes.len() > CONTROL_FRAME_MAX || bytes.last() == Some(&b'\n') {
            return Err(
                "custody packet must be nonempty JCS without LF and at most 16384 bytes"
                    .to_string(),
            );
        }
        ...
        let number = |key: &str| -> Result<u64, String> {
            ...
            if !n.is_finite() || n < 0.0 || n.fract() != 0.0 || n > 9_007_199_254_740_991.0 {
                return Err(format!(
                    "custody packet {key} is not a safe unsigned integer"
                ));
            }
            Ok(n as u64)
        };
...
impl ControlEndpoint {
    #[cfg(target_os = "linux")]
    pub fn pair() -> Result<(OwnedFd, OwnedFd), String> {
        let mut fds = [-1; 2];
        let rc = unsafe {
            libc::socketpair(
                libc::AF_UNIX,
    ...
    pub fn new(
        ...
        #[cfg(target_os = "linux")]
        let peer_pidfd = crate::workspace::platform::linux::pidfd_open(expected_peer.pid)?;
        Ok(Self {
            fd,
            key,
            session_id,
    ...
    pub fn send(
        ...
        self.tx_seq = self
            .tx_seq
            .checked_add(1)
            .ok_or_else(|| "custody send sequence overflow".to_string())?;
    ...
    pub fn receive(
        ...
        self.receive_admitted_fds(timeout, &[expected_fd_count])
    }

    #[cfg(target_os = "linux")]
    fn receive_admitted_fds(
        &mut self,
        timeout: Duration,
        admitted_fd_counts: &[usize],
    ) -> Result<ReceivedPacket, String> {
        wait_readable(self.fd.as_raw_fd(), timeout)?;
        let RecvmsgPacket {
            bytes,
            credentials,
            fds,
        } = recvmsg_packet(self.fd.as_raw_fd())?;
        let credentials =
            credentials.ok_or_else(|| "custody packet has no SCM_CREDENTIALS".to_string())?;
        ...
        let current_start =
            crate::workspace::platform::linux::process_start_token(credentials.pid)?;
        if current_start != self.expected_peer.start_token {
            return Err("custody packet peer start token changed".to_string());
        }
        if !admitted_fd_counts.contains(&fds.len()) {
            return Err(format!(
                "custody packet has {} FDs; expected one of {admitted_fd_counts:?}",
                fds.len()
            ));
        }
    ...
    pub fn command(
        ...
        if command.is_heartbeat() || expected_ack.is_heartbeat() {
    ...
    pub fn heartbeat(&mut self) -> Result<(), String> {
        let seq = match self.pending_heartbeat_seq {
            Some(seq) => seq,
            None => {
                let seq = self.send(PacketKind::Heartbeat, ControlPayload::new(), &[])?;
        ...
        match self.receive(HEARTBEAT_INTERVAL, 0) {
            ...
            Err(error) if error.starts_with("custody control deadline elapsed after ") => {
                self.missed_heartbeats = self.missed_heartbeats.saturating_add(1);
                if self.missed_heartbeats >= 3 {
                    Err(HEARTBEAT_FENCE_ERROR.to_string())
                } else {
                    Ok(())
                }
            }
    ...
    pub fn acknowledge(
        ...
        if let Some(evidence) = evidence_sha256 {
            ...
            payload.insert(
                ...
                PayloadValue::String(evidence.to_string()),
            );
        }
...
pub struct CustodyController {
    ...
    phase: CustodyPhase,
    #[cfg(target_os = "linux")]
    pending_bootstrap_fault: Option<ReceivedPacket>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RetainedBootstrapFault {
    pub code: String,
    pub evidence_sha256: String,
...
#[cfg(target_os = "linux")]
fn decode_worker_prepared(packet: ReceivedPacket) -> Result<ReceivedWorkerRoot, String> {
    if packet.packet.kind != PacketKind::WorkerPrepared || packet.packet.payload.len() != 4 {
        return Err("custodian did not send exact WORKER_PREPARED".to_string());
    }
    let string = |key: &str| {
        packet
            .packet
    ...
    let pid = match packet.packet.payload.get("pid") {
        Some(PayloadValue::Unsigned(pid)) if *pid > 0 && *pid <= libc::pid_t::MAX as u64 => {
            *pid as libc::pid_t
        }
    ...
    let cgroup_membership = string("cgroup_membership")
        .ok_or_else(|| "WORKER_PREPARED cgroup membership is missing".to_string())?;
    if !cgroup_membership.starts_with('/') {
        return Err("WORKER_PREPARED cgroup membership is not absolute".to_string());
    }
    let expected_evidence = worker_prepared_evidence(pid, &start_token, &cgroup_membership)?;
    if !constant_time_eq(evidence.as_bytes(), expected_evidence.as_bytes()) {
        return Err("WORKER_PREPARED evidence does not bind its identity".to_string());
    }
...
impl CustodyController {
    ...
    pub fn bootstrap(
        ...
        let seq = self.begin_bootstrap(payload, fds)?;
        self.finish_bootstrap(seq)
    }
    ...
    pub fn begin_bootstrap(
        ...
        self.endpoint.send(PacketKind::Bootstrap, payload, fds)
    }
    ...
    pub fn worker_prepared(&mut self) -> Result<ReceivedWorkerRoot, String> {
        ...
        if packet.packet.kind == PacketKind::Fault && packet.fds.is_empty() {
            ...
            return Err(format!(
                "worker preparation retained fault {code} ({evidence_sha256})"
            ));
        }
        ...
        let root = match decode_worker_prepared(packet) {
            Ok(root) => root,
            Err(error) => {
                let evidence = custody_fault_evidence("worker_prepared_invalid", &error);
    ...
    pub fn activate(&mut self) -> Result<ControlPayload, String> {
        ...
        while self.endpoint.pending_heartbeat_seq.is_some() {
        ...
        if received.packet.kind == PacketKind::Fault {
            ...
            return Err(format!(
                "activation retained fault {code} ({evidence_sha256})"
            ));
        }
        if received.packet.kind != PacketKind::Activated {
            return Err(format!(
                "custody command ACTIVATE received {}, expected ACTIVATED",
                received.packet.kind.as_str()
            ));
        }
        match validate_ack(&received.packet.payload, seq, true)? {
            AckStatus::Ok => {}
            AckStatus::Fault {
                code,
                evidence_sha256,
                ...
                return Err(format!(
                    "custody command ACTIVATE received fault ACK {code} ({evidence_sha256})"
                ));
            }
        }
    ...
    #[cfg(target_os = "linux")]
    pub fn accept_bootstrap_fault(&mut self) -> Result<RetainedBootstrapFault, String> {
        if !matches!(
            self.phase,
            CustodyPhase::Bootstrapping | CustodyPhase::Ready | CustodyPhase::Prepared
        ) {
            return Err(format!(
                "bootstrap fault recovery is not admitted in {:?}",
                self.phase
            ));
        }
        ...
        if fault.packet.kind != PacketKind::Fault {
            return Err(format!(
                "bootstrap fault recovery received {}; expected FAULT",
                fault.packet.kind.as_str(),
            ));
        }
        ...
        let empty_sha256 = match (
            ...
            empty.packet.payload.get("evidence_sha256"),
        ) {
            (PacketKind::Empty, 1, Some(PayloadValue::String(value))) => {
                validate_sha256(value)?;
                value.clone()
            }
            _ => {
                return Err(
                    "bootstrap fault recovery did not receive exact EMPTY evidence".to_string(),
                );
            }
        };
        self.endpoint
            .acknowledge(&empty.packet, PacketKind::Empty, Some(&empty_sha256))?;
        self.phase = CustodyPhase::Empty;
        Ok(RetainedBootstrapFault {
            code,
            evidence_sha256,
            empty_sha256,
        })
    ...
    pub fn heartbeat(&mut self) -> Result<(), String> {
        if !matches!(
            ...
            CustodyPhase::Active
                | CustodyPhase::Empty
                | CustodyPhase::ReleasePrepared
                | CustodyPhase::LeaseClosed
        ) {
    ...
    pub fn confirm_empty(&mut self, evidence_sha256: &str) -> Result<(), String> {
        ...
        {
            let error = "QUIESCED custody did not provide exact EMPTY evidence";
            let fault_evidence = custody_fault_evidence("empty_invalid", error);
            return match self.endpoint.acknowledge_fault(
                &empty.packet,
                PacketKind::Empty,
                "empty_invalid",
        ...
        self.endpoint
            .acknowledge(&empty.packet, PacketKind::Empty, Some(evidence_sha256))?;
        self.phase = CustodyPhase::Empty;
        Ok(())
    }
    ...
    fn empty_command(
        ...
        let payload = self
            .endpoint
            .command(command, ControlPayload::new(), response, true)
            .inspect_err(|_| {
                self.phase = CustodyPhase::Faulted;
            })?;
        self.phase = CustodyPhase::Empty;
...
impl CustodianServer {
    ...
    pub fn next_admitted(
        ...
        loop {
            ...
            if packet.packet.kind == PacketKind::Heartbeat {
                ...
                self.endpoint
                    .acknowledge(&packet.packet, PacketKind::HeartbeatAck, None)?;
                continue;
            }
            if packet.packet.kind == PacketKind::Fault {
                let (code, evidence) = validate_fault_payload(&packet.packet.payload)?;
                self.endpoint
                    .acknowledge(&packet.packet, PacketKind::Fault, Some(&evidence))?;
                return Err(format!("peer sent authenticated FAULT {code} ({evidence})"));
            }
    ...
    pub fn acknowledge_fault(
        ...
        self.endpoint
            .acknowledge_fault(command, response, code, evidence_sha256)?;
        Ok(())
    }
    ...
    #[cfg(target_os = "linux")]
    pub fn send_fault(&mut self, code: &str, evidence_sha256: &str) -> Result<u64, String> {
        self.endpoint.send(
            PacketKind::Fault,
            fault_payload(code, evidence_sha256)?,
            &[],
        )
    }
}
...
pub fn set_fd_inheritable_for_exec(fd: RawFd, inheritable: bool) -> Result<(), String> {
...
impl LeaseReference {
    ...
    pub fn close(mut self) -> LeaseCloseEvidence {
        drop(self.fd.take());
        let evidence_sha256 =
            hex_digest(format!("custodian-lease-close-v1\0{}\0{}", self.dev, self.ino).as_bytes());
        LeaseCloseEvidence {
            dev: self.dev,
            ino: self.ino,
            evidence_sha256,
...
pub fn fault_payload(code: &str, evidence_sha256: &str) -> Result<ControlPayload, String> {
    if code.is_empty()
        || !code
            .bytes()
...
pub fn evidence_payload(evidence_sha256: &str) -> Result<ControlPayload, String> {
    validate_sha256(evidence_sha256)?;
    Ok(BTreeMap::from([(
        "evidence_sha256".to_string(),
        PayloadValue::String(evidence_sha256.to_string()),
...
pub fn worker_prepared_evidence(
    ...
    Ok(hex_digest(
        format!("worker-prepared-v1\0{pid}\0{start_token}\0{cgroup_membership}\0sandbox=1")
            .as_bytes(),
    ))
}
...
fn validate_identity(session_id: &str, generation: u64, seq: u64) -> Result<(), String> {
    ...
    if bytes.len() != 36
        ...
        || bytes
            .iter()
            .enumerate()
            .any(|(i, b)| !matches!(i, 8 | 13 | 18 | 23) && !matches!(b, b'0'..=b'9' | b'a'..=b'f'))
    {
        return Err("custody session_id is not lowercase UUIDv4".to_string());
    }
    if generation == 0 || seq == 0 {
        return Err("custody generation and sequence must be positive".to_string());
    }
    Ok(())
}
...
fn validate_fault_payload(payload: &ControlPayload) -> Result<(String, String), String> {
    if payload.len() != 2 {
    ...
    let evidence_sha256 = match payload.get("evidence_sha256") {
        Some(PayloadValue::String(evidence_sha256)) => evidence_sha256.clone(),
    ...
    match payload.get("status") {
        ...
        Some(PayloadValue::String(status)) if status == "fault" => {
            ...
            let evidence_sha256 = match payload.get("evidence_sha256") {
                Some(PayloadValue::String(evidence_sha256)) => evidence_sha256.clone(),
                _ => {
                    return Err("custody fault ACK evidence is missing".to_string());
                }
            };
            validate_sha256(&evidence_sha256)?;
            Ok(AckStatus::Fault {
...
fn decode_payload(value: &JsonValue) -> Result<ControlPayload, String> {
    let object = value
        .get::<HashMap<String, JsonValue>>()
        .ok_or_else(|| "custody payload is not an object".to_string())?;
    let mut payload = ControlPayload::new();
    for (key, value) in object {
        let decoded = if let Some(value) = value.get::<String>() {
            ...
            return Err("custody payload contains a nested or unsupported value".to_string());
        };
...
fn decode_hex_32(value: &str) -> Result<[u8; 32], String> {
    if value.len() != 64 {
        return Err("custody MAC is not 32-byte lowercase hex".to_string());
    }
    let mut out = [0; 32];
    for (index, chunk) in value.as_bytes().chunks_exact(2).enumerate() {
        ...
        out[index] = digit(chunk[0])
            .zip(digit(chunk[1]))
            .map(|(hi, lo)| (hi << 4) | lo)
            .ok_or_else(|| "custody MAC is not lowercase hex".to_string())?;
    }
    Ok(out)
}
...
fn sendmsg_packet(fd: RawFd, bytes: &[u8], rights: &[RawFd]) -> Result<(), String> {
    if rights.len() > 4 {
        return Err("custody packet carries more than four FDs".to_string());
    }
...
#[cfg(target_os = "linux")]
fn recvmsg_packet(fd: RawFd) -> Result<RecvmsgPacket, String> {
...
fn wait_readable(fd: RawFd, timeout: Duration) -> Result<(), String> {
    ...
    loop {
        ...
        if rc == 0 {
            return Err(format!(
                "custody control deadline elapsed after {} ms",
                started.elapsed().as_millis()
            ));
        }
        let error = std::io::Error::last_os_error();
        if error.kind() != std::io::ErrorKind::Interrupted {
            return Err(format!("poll custody control: {error}"));
        }
    }
}
...
mod tests {
    ...
    fn late_heartbeat_ack_satisfies_the_outstanding_deadline() {
        ...
        let responder = std::thread::spawn(move || {
            ...
            supervisor_endpoint
                .acknowledge(&heartbeat.packet, PacketKind::HeartbeatAck, None)
                .unwrap();
        });
        let mut controller = CustodyController {
    ...
    fn worker_prepared_uses_one_shared_preimage() {
        ...
        let evidence = worker_prepared_evidence(pid, &start_token, membership).unwrap();
        let (guardian_fd, supervisor_fd) = ControlEndpoint::pair().unwrap();
        let key = [0x63; 32];
        let session_id = "00000000-0000-4000-8000-000000000001".to_string();
        let guardian_endpoint = ControlEndpoint::new(
        ...
        let sender = std::thread::spawn(move || {
            let pidfd = crate::workspace::platform::linux::pidfd_open(pid).unwrap();
            ...
            let seq = server
                ...
                .unwrap();
            server
                .wait_ack(PacketKind::WorkerPrepared, seq, Some(&expected_evidence))
                .unwrap();
        });
        let mut controller = CustodyController {
            endpoint: guardian_endpoint,
    ...
    fn fault_ack_is_exact_and_authenticated_by_packet_mac() {
        ...
        let payload = fault_ack_payload(7, "activation_failed", &evidence).unwrap();
        let key = [0x5a; 32];
        let mut packet = ControlPacket {
        ...
        let decoded = ControlPacket::decode(&packet.encode().unwrap(), &key).unwrap();
        assert_eq!(
    ...
    fn bootstrap_fault_ack_is_rejected_without_phase_progress() {
        ...
        let responder = std::thread::spawn(move || {
            let command = supervisor_endpoint
                ...
                .unwrap();
            let evidence = custody_fault_evidence("bootstrap_invalid", "bad bootstrap");
            supervisor_endpoint
                .acknowledge_fault(
                    &command.packet,
    ...
    #[test]
    fn bootstrap_fault_is_authenticated_and_transitions_to_empty() {
        let peer = current_peer();
        let (guardian_fd, supervisor_fd) = ControlEndpoint::pair().unwrap();
        let key = [0x72; 32];
        let session_id = "00000000-0000-4000-8000-000000000001".to_string();
        ...
        let supervisor_endpoint =
            ControlEndpoint::new(supervisor_fd, key, session_id, 1, Sender::Supervisor, peer)
                .unwrap();
        let evidence = custody_fault_evidence("worker_preparation_failed", "injected");
        let empty = "b".repeat(64);
        let expected_evidence = evidence.clone();
        let expected_empty = empty.clone();
        let responder = std::thread::spawn(move || {
            let mut server = CustodianServer::new(supervisor_endpoint);
            let command = server.next_command(PacketKind::Bootstrap, 4).unwrap();
            server
                .acknowledge(
                    &command.packet,
                    PacketKind::SupervisorReady,
                ...
                .unwrap();
            let fault_seq = server
                .send_fault("worker_preparation_failed", &expected_evidence)
                .unwrap();
            server
                .wait_ack(PacketKind::Fault, fault_seq, Some(&expected_evidence))
                .unwrap();
            let empty_seq = server.send_empty(&expected_empty).unwrap();
            server
                .wait_ack(PacketKind::Empty, empty_seq, Some(&expected_empty))
                .unwrap();
            let prepare = server.next_command(PacketKind::PrepareRelease, 0).unwrap();
            server
                .acknowledge(
                    &prepare.packet,
                    PacketKind::ReleasePrepared,
                ...
                .unwrap();
            let close = server.next_command(PacketKind::CloseLease, 0).unwrap();
            server
                .acknowledge(&close.packet, PacketKind::LeaseClosed, &"d".repeat(64))
                .unwrap();
            let committed = server.next_command(PacketKind::ClosedCommitted, 0).unwrap();
            server
                .acknowledge(
                    &committed.packet,
                    PacketKind::ClosedCommitted,
        ...
        let mut controller = CustodyController::new(guardian_endpoint);
        let seq = controller
            .begin_bootstrap(ControlPayload::new(), &fds)
            .unwrap();
        controller.finish_bootstrap(seq).unwrap();
        assert!(controller.worker_prepared().is_err());

        let retained = controller.accept_bootstrap_fault().unwrap();
        assert_eq!(retained.code, "worker_preparation_failed");
        assert_eq!(retained.evidence_sha256, evidence);
        assert_eq!(retained.empty_sha256, empty);
        assert_eq!(controller.phase(), CustodyPhase::Empty);
        assert_eq!(
            controller.prepare_release().unwrap().get("evidence_sha256"),
            Some(&PayloadValue::String("c".repeat(64))),
        );
        assert_eq!(
            controller.close_lease().unwrap().get("evidence_sha256"),
            Some(&PayloadValue::String("d".repeat(64))),
        );
        controller.closed_committed(&"e".repeat(64)).unwrap();
        assert_eq!(controller.phase(), CustodyPhase::ClosedCommitted);
        responder.join().unwrap();
    }
}
