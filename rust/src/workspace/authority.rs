...
use super::schema::{
    self, CapabilityRecordV1, ClosedJcs, CoordinatorCapabilityV1, JcsValue, PathClaimRequestV1,
...
impl IntegrationQueueLock {
    pub fn acquire(repository_dir: &Path) -> Result<Self, String> {
        ...
        let (file, identity, rank) = repository_gate::acquire_ranked_lock(
            &path,
            euid,
            "IntegrationQueueLock",
            ShortLockRank::IntegrationQueue,
        )?;
        Ok(Self {
            file,
            path,
    ...
    pub fn revalidate(&self) -> Result<(), String> {
        repository_gate::revalidate_ranked_lock(
            &self.file,
            &self.path,
            self.identity,
            self.euid,
...
impl AuthorityExclusionLock {
    pub fn acquire(repository_dir: &Path) -> Result<Self, String> {
        ...
        let (file, identity, rank) = repository_gate::acquire_ranked_lock(
            &path,
            euid,
            "WorkspaceAuthorityExclusionLock",
            ShortLockRank::AuthorityExclusion,
        )?;
        Ok(Self {
            file,
            path,
    ...
    pub fn contains(&self, path: &Path) -> bool {
        path.starts_with(&self.repository_dir)
    }
}
...
impl WorkspaceJournalLock {
    pub fn acquire(session_dir: &Path) -> Result<Self, String> {
        ...
        let (file, identity, rank) = repository_gate::acquire_ranked_lock(
            &path,
            euid,
            "WorkspaceJournalLock",
            ShortLockRank::WorkspaceJournal,
        )?;
        Ok(Self {
            file,
            path,
...
impl ClosedJcs for RepositoryAuthorityV1 {
    fn from_jcs(value: JcsValue) -> Result<Self, String> {
        ...
        let string = |value: Option<&JcsValue>, name: &str| {
            value
                .ok_or_else(|| format!("missing {name}"))?
                .as_str()
                .map(str::to_string)
        };
        if string(object.get("schema"), "schema")? != schema::SCHEMA_V1 {
            return Err("RepositoryAuthorityV1 schema mismatch".to_string());
        }
        ...
        if coordinator.generation != current_generation {
            return Err("authority coordinator generation mismatch".to_string());
        }
...
impl WorkspaceAuthority {
    pub fn system() -> Result<Self, String> {
        Self::new(SystemAuthorityRootProvider.roots()?)
    }
    pub fn new(roots: AuthorityRoots) -> Result<Self, String> {
        if roots.euid != unsafe { libc::geteuid() } {
            return Err("authority root EUID differs from the process EUID".to_string());
        }
    ...
    pub fn roots(&self) -> &AuthorityRoots {
        &self.roots
    }
    pub fn repository_dir(&self, repository_id: &str) -> Result<PathBuf, String> {
    ...
    pub fn capability_path(&self, repository_id: &str, generation: u64) -> Result<PathBuf, String> {
        Ok(self
            .repository_dir(repository_id)?
            .join("coordinator-capabilities")
            .join(format!("{generation:020}.json")))
    }
    pub fn acquire_integration_queue(
        &self,
        repository_id: &str,
    ) -> Result<IntegrationQueueLock, String> {
        IntegrationQueueLock::acquire(&self.repository_dir(repository_id)?)
    }
    pub fn acquire_authority_exclusion(
        &self,
        repository_id: &str,
    ) -> Result<AuthorityExclusionLock, String> {
        AuthorityExclusionLock::acquire(&self.repository_dir(repository_id)?)
    }

    pub fn bootstrap_coordinator(
        &self,
        ...
        if target.exists() {
            return Err(format!(
                "workspace authority already exists; retry with --coordinator-capability-file {}",
                deterministic.display()
            ));
        }
        let repositories = self.roots.authority.join("repositories");
        let stage = repositories.join(format!(
            ".bootstrap-{}-{}",
        ...
        let result = (|| {
            for relative in ["coordinator-capabilities", "sessions", "journal"] {
                create_private_directory(&stage.join(relative), self.roots.euid)?;
            }
            let (capability, record) =
                capability::mint_coordinator(&repository.repository_id, 1, now)?;
            let authority = RepositoryAuthorityV1 {
                repository: repository.clone(),
                current_generation: "1".into(),
                coordinator: record,
                created_at: now.into(),
                updated_at: now.into(),
            };
            ...
            atomic_create_jcs(
                &stage.join("coordinator-capabilities/00000000000000000001.json"),
                &capability,
                0o600,
            )?;
            ...
            Ok(BootstrapResult {
                capability,
                capability_file: deterministic,
                bootstrap: BootstrapOutcome::Created,
            })
        })();
    ...
    pub fn authenticate(
        ...
        action: &str,
    ) -> Result<(RepositoryAuthorityV1, CoordinatorCapabilityV1), String> {
        let authority = self.read_repository(repository_id)?;
        let generation = authority
            .current_generation
            .parse::<u64>()
            .map_err(|_| "authority generation overflow".to_string())?;
        let expected = self.capability_path(repository_id, generation)?;
        let canonical = fs::canonicalize(capability_path)
            .map_err(|error| format!("canonicalize coordinator capability: {error}"))?;
        if canonical != expected {
            return Err(format!(
                "coordinator capability must be the exact current path {}",
                expected.display()
            ));
        }
    ...
    pub fn rotate_coordinator_cas(
        ...
        schema::Timestamp::parse(now)?;
        let repository_dir = self.repository_dir(repository_id)?;
        if exclusion.repository_dir() != repository_dir {
            return Err(
                "coordinator rotation authority exclusion has the wrong repository identity".into(),
            );
        }
        exclusion.revalidate()?;
        let (expected, current_capability) =
            self.authenticate(repository_id, current_path, "recover")?;
        ...
        let next = current
            .checked_add(1)
            .ok_or_else(|| "coordinator generation exhausted".to_string())?;
        ...
        fsync_directory(
            path.parent()
                .ok_or_else(|| "coordinator capability path has no parent".to_string())?,
        )?;

        exclusion.revalidate()?;
        let observed = self.read_repository(repository_id)?;
        if observed != expected {
            return Err("coordinator authority changed after next-generation publication; refusing CAS replacement".into());
        }
        capability::authenticate_coordinator(
            &current_capability,
        ...
        if durable != replacement {
            return Err("coordinator authority CAS publication is not durably re-readable".into());
        }
        Ok(BootstrapResult {
            capability,
            capability_file: path,
            bootstrap: BootstrapOutcome::Existing,
        })
    }
    ...
    pub fn read_repository(&self, repository_id: &str) -> Result<RepositoryAuthorityV1, String> {
        ...
        if authority.repository.repository_id != repository_id {
            return Err("authority repository ID mismatch".to_string());
        }
        Ok(authority)
    }
}
...
impl ClosedJcs for LeaseOwnerV1 {
    fn from_jcs(value: JcsValue) -> Result<Self, String> {
        ...
        let keys = ["schema", "session_id", "acquired_at"];
        if object.len() != keys.len() || keys.iter().any(|key| !object.contains_key(*key)) {
            return Err("LeaseOwnerV1 keys differ".into());
        }
        let session_id = object["session_id"].as_str()?.to_string();
        schema::LowerUuidV4::parse(&session_id)?;
        let acquired_at = object["acquired_at"].as_str()?.to_string();
    ...
    fn to_jcs(&self) -> JcsValue {
        JcsValue::Object(BTreeMap::from([
            (
                "acquired_at".into(),
                JcsValue::String(self.acquired_at.clone()),
            ),
            ("schema".into(), JcsValue::String(schema::SCHEMA_V1.into())),
            (
                "session_id".into(),
...
impl WorkspaceLease {
            ...
            Err(error) if error == "workspace lease is already held" => {
                ...
                loop {
                    if let Ok(bytes) = read_secure_bytes(owner_path) {
                        if let Ok(value) = schema::parse_jcs(&bytes, true) {
                            if let Ok(owner) = LeaseOwnerV1::from_jcs(value) {
                                return Err(format!(
                                    "Workspace already owned by session {}. Open a separate worktree or continue in read-only mode.",
                                    owner.session_id
                                ));
                            }
                        }
                    ...
                    if std::time::Instant::now() >= deadline {
                        return Err("workspace lease is held but exact owner evidence is not durably readable; refusing recovery".into());
                    }
    ...
    pub fn duplicate(&self) -> Result<File, String> {
        self.file
            .try_clone()
            .map_err(|error| format!("duplicate workspace lease: {error}"))
    }
    ...
    pub fn identity(&self) -> Result<LeaseIdentity, String> {
        let metadata = self
            .file
            .metadata()
            .map_err(|error| format!("fstat workspace lease {}: {error}", self.path.display()))?;
        Ok(LeaseIdentity {
            device: metadata.dev(),
            inode: metadata.ino(),
        })
...
impl WorkspaceLeaseProbe {
    ...
    pub fn revalidate(&self) -> Result<(), String> {
        ...
        let opened = self
            .file
            .metadata()
            .map_err(|error| format!("fstat held workspace lease probe: {error}"))?;
        let named = fs::symlink_metadata(&self.path)
            .map_err(|error| format!("revalidate held workspace lease probe path: {error}"))?;
        let exact = |metadata: &fs::Metadata| {
            metadata.is_file()
                && metadata.uid() == euid
                && metadata.nlink() == 1
        ...
        if !exact(&opened) || !exact(&named) {
            return Err(
                "held workspace lease probe no longer excludes the exact durable lease inode"
                    .into(),
            );
        }
        Ok(())
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct JournalEventV1 {
    pub sequence: u64,
    pub previous_sha256: Option<String>,
    pub kind: String,
    pub payload: JcsValue,
    pub created_at: String,
}
impl ClosedJcs for JournalEventV1 {
    fn from_jcs(value: JcsValue) -> Result<Self, String> {
        ...
        if o.len() != keys.len() || keys.iter().any(|k| !o.contains_key(*k)) {
            return Err("JournalEventV1 keys differ".into());
        }
        if o["schema"].as_str()? != schema::SCHEMA_V1 {
            return Err("journal schema mismatch".into());
        }
        let sequence = o["sequence"]
            .as_str()?
            .parse()
            .map_err(|_| "invalid journal sequence".to_string())?;
        let previous_sha256 = match &o["previous_sha256"] {
            JcsValue::Null => None,
            JcsValue::String(v) => {
                Sha256Digest::parse(v)?;
                Some(v.clone())
            }
            _ => return Err("invalid journal previous hash nullability".into()),
        };
        let created_at = o["created_at"].as_str()?.to_string();
        schema::Timestamp::parse(&created_at)?;
        Ok(Self {
            sequence,
            previous_sha256,
    ...
    fn to_jcs(&self) -> JcsValue {
        JcsValue::Object(BTreeMap::from([
            ...
            ("kind".into(), JcsValue::String(self.kind.clone())),
            ...
            (
                "sequence".into(),
                JcsValue::String(self.sequence.to_string()),
            ),
        ]))
    }
}
...
pub fn read_journal_position(session_dir: &Path) -> Result<JournalPosition, String> {
    ...
    for entry in fs::read_dir(&journal)
        .map_err(|error| format!("read workspace journal {}: {error}", journal.display()))?
    {
        let entry = entry.map_err(|error| format!("read workspace journal entry: {error}"))?;
        let name = entry
            .file_name()
            .into_string()
            .map_err(|_| "workspace journal contains a non-UTF-8 entry".to_string())?;
        if name.len() != 25
            || !name.ends_with(".json")
            || !name[..20].bytes().all(|byte| byte.is_ascii_digit())
        {
            return Err(format!(
                "workspace journal contains unexpected entry {name}"
            ));
        }
        let sequence = name[..20]
            .parse::<u64>()
            .map_err(|_| "workspace journal sequence overflows u64".to_string())?;
        if sequence == 0 {
            return Err("workspace journal sequence must begin at one".into());
        }
    ...
    for (index, (sequence, path)) in entries.iter().enumerate() {
        ...
        if *sequence != expected {
            return Err("workspace journal is not a contiguous sequence".into());
        }
        let bytes = read_secure_bytes(path)?;
        let event = JournalEventV1::from_jcs(schema::parse_jcs(&bytes, true)?)?;
        if event.sequence != *sequence || event.previous_sha256 != head {
            return Err(
                "workspace journal event sequence or hash chain differs from durable position"
                    .into(),
            );
        }
...
pub fn append_journal(
    ...
    if event.sequence != expected_sequence {
        return Err(
            "journal event sequence differs from the requested publication sequence".into(),
        );
    }
    let current = expected_sequence
        .checked_sub(1)
        .ok_or_else(|| "journal sequence must begin at one".to_string())?;
    let lock = WorkspaceJournalLock::acquire(session_dir)?;
    append_journal_cas(&lock, event, current, expected_head)
}

pub fn append_journal_cas(
    lock: &WorkspaceJournalLock,
    event: &JournalEventV1,
    expected_current_sequence: u64,
    expected_head: Option<&str>,
) -> Result<String, String> {
    lock.revalidate()?;
    let next = expected_current_sequence
        .checked_add(1)
        .ok_or_else(|| "journal sequence exhausted".to_string())?;
    if event.sequence != next || event.previous_sha256.as_deref() != expected_head {
        return Err("journal CAS sequence/head mismatch".into());
    }
    let durable = read_journal_position(lock.session_dir())?;
    if durable.sequence != expected_current_sequence || durable.head.as_deref() != expected_head {
        return Err("durable journal position changed before publication".into());
    }
    let path = lock
        .session_dir()
        .join("journal")
        .join(format!("{:020}.json", event.sequence));
    atomic_create_jcs(&path, event, 0o600)?;
    let digest = schema::jcs_sha256(event);
    let published = read_journal_position(lock.session_dir())?;
    if published.sequence != event.sequence || published.head.as_deref() != Some(digest.as_str()) {
        return Err("published journal event is not durably re-readable".into());
    }
    Ok(digest)
}

pub fn replace_manifest_cas<T: ClosedJcs>(
    lock: &WorkspaceJournalLock,
    path: &Path,
    expected_state: &str,
    expected_sequence: u64,
    ...
    if path.parent() != Some(lock.session_dir()) {
        return Err("manifest CAS path is outside the locked workspace session".into());
    }
    lock.revalidate()?;
    ...
    if state != expected_state || sequence != expected_sequence || head.as_deref() != expected_head
    {
        return Err("manifest CAS state/sequence/head changed before publication".into());
    }
    ...
    if replacement_sequence != journal.sequence || replacement_head != journal.head {
        return Err("replacement manifest does not name the exact durable journal position".into());
    }
    lock.revalidate()?;
    atomic_replace_jcs(path, replacement, 0o600)
}

pub struct ClosedManifestCas<'a, T: ClosedJcs> {
    pub gate: &'a repository_gate::RepositoryGate,
    pub roots: &'a AuthorityRoots,
    pub exclusion: &'a AuthorityExclusionLock,
    pub probe: &'a WorkspaceLeaseProbe,
    pub lock: &'a WorkspaceJournalLock,
    pub path: &'a Path,
...
pub fn commit_closed_manifest_cas<T: ClosedJcs>(
    context: ClosedManifestCas<'_, T>,
) -> Result<(), String> {
    let ClosedManifestCas {
        gate,
        roots,
        exclusion,
        probe,
    ...
    let repository_id = exclusion
        .repository_dir()
        .file_name()
        .and_then(OsStr::to_str)
        .ok_or_else(|| "authority exclusion repository path has no UTF-8 identity".to_string())?;
    if gate.repository_id() != repository_id {
        return Err(
            "RepositoryGate and authority exclusion identify different repositories".into(),
        );
    }
    let (state, _, _) = manifest_cas_position(&replacement.to_jcs())?;
    if state != "Closed" {
        return Err("lease probe publication is restricted to the terminal Closed manifest".into());
    }
    gate.revalidate_lock(roots)?;
    exclusion.revalidate()?;
    probe.revalidate()?;
    replace_manifest_cas(
        lock,
        path,
        expected_state,
        expected_sequence,
...
fn manifest_cas_position(value: &JcsValue) -> Result<(String, u64, Option<String>), String> {
    let object = match value {
        ...
        _ => return Err("workspace manifest must be an object".into()),
    };
    let state = object
        .get("state")
        .ok_or_else(|| "workspace manifest has no state".to_string())?
        .as_str()?
        .to_string();
    let sequence_text = object
        .get("journal_sequence")
        .ok_or_else(|| "workspace manifest has no journal_sequence".to_string())?
        .as_str()?;
    schema::Decimal::parse(sequence_text)?;
    let sequence = sequence_text
        .parse::<u64>()
        .map_err(|_| "workspace manifest journal_sequence overflows u64".to_string())?;
    let head = match object
        .get("journal_head_sha256")
        .ok_or_else(|| "workspace manifest has no journal_head_sha256".to_string())?
    {
        JcsValue::Null => None,
    ...
    if (sequence == 0) != head.is_none() {
        return Err("workspace manifest journal sequence/head nullability mismatch".into());
    }
    Ok((state, sequence, head))
}

pub fn atomic_create_jcs<T: ClosedJcs>(path: &Path, value: &T, mode: u32) -> Result<(), String> {
    atomic_write(path, &schema::serialize_jcs_lf(value), mode, false)
}
pub fn atomic_replace_jcs<T: ClosedJcs>(path: &Path, value: &T, mode: u32) -> Result<(), String> {
    atomic_write(path, &schema::serialize_jcs_lf(value), mode, true)
}
fn atomic_write(path: &Path, bytes: &[u8], mode: u32, replace: bool) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or_else(|| "record path has no parent".to_string())?;
    let temp = parent.join(format!(".tmp-{}", crate::store::uuid_v4()));
    ...
    file.write_all(bytes)
        ...
        .map_err(|e| format!("persist {}: {e}", temp.display()))?;
    drop(file);
    let result = if replace {
        fs::rename(&temp, path).map_err(|e| format!("replace {}: {e}", path.display()))
    } else {
        rename_noreplace(&temp, path)
            .map_err(|e| format!("create-once publish {}: {e}", path.display()))
    };
    if result.is_err() {
...
fn verify_existing_private_directory(path: &Path, euid: u32) -> Result<(), String> {
    ...
    let file = OpenOptions::new()
        ...
        .map_err(|error| {
            format!(
                "securely open existing private directory {}: {error}",
                path.display()
            )
        })?;
    let metadata = file.metadata().map_err(|error| {
        format!(
            "fstat existing private directory {}: {error}",
            path.display()
        )
    })?;
    let named = fs::symlink_metadata(path).map_err(|error| {
        format!(
            "revalidate existing private directory {}: {error}",
            path.display()
        )
    })?;
    if !metadata.is_dir()
    ...
    {
        return Err(format!(
            "{} is not an exact EUID-owned mode-0700 real directory",
            path.display()
        ));
    }
    Ok(())
...
pub fn ensure_private_directory(path: &Path, euid: u32) -> Result<(), String> {
        ...
        match fs::symlink_metadata(&current) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                ...
                fs::set_permissions(&current, fs::Permissions::from_mode(0o700))
                    .map_err(|e| format!("chmod {}: {e}", current.display()))?;
            }
            Err(error) => {
                return Err(format!(
                    "inspect private directory component {}: {error}",
                    current.display()
                ));
            }
        }
    ...
    let file = OpenOptions::new()
        ...
        .custom_flags(libc::O_CLOEXEC | libc::O_NOFOLLOW | libc::O_DIRECTORY)
        .open(path)
        .map_err(|e| format!("securely open directory {}: {e}", path.display()))?;
    let metadata = file
        .metadata()
        .map_err(|e| format!("fstat {}: {e}", path.display()))?;
    if !metadata.is_dir() || metadata.uid() != euid || metadata.mode() & 0o777 != 0o700 {
        return Err(format!(
            "{} is not an EUID-owned mode-0700 directory",
            path.display()
        ));
    }
    Ok(())
}
fn create_private_directory(path: &Path, euid: u32) -> Result<(), String> {
    fs::create_dir(path)
        .map_err(|e| format!("create private directory {}: {e}", path.display()))?;
    fs::set_permissions(path, fs::Permissions::from_mode(0o700))
        .map_err(|e| format!("chmod {}: {e}", path.display()))?;
    ensure_private_directory(path, euid)
}
fn verify_regular(file: &File, path: &Path, mode: u32, euid: u32) -> Result<(), String> {
    let m = file
        .metadata()
        .map_err(|e| format!("fstat {}: {e}", path.display()))?;
    if !m.is_file() || m.uid() != euid || m.nlink() != 1 || m.mode() & 0o777 != mode {
        return Err(format!(
            "{} is not an EUID-owned single-link mode-{mode:o} file",
            path.display()
        ));
    }
    Ok(())
    ...
    match rename_noreplace(stage, target) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            fs::remove_dir_all(stage).map_err(|cleanup| {
                format!(
                    "remove losing bootstrap staging directory {}: {cleanup}",
                    stage.display()
                )
            })?;
            Err(format!(
...
pub fn repository_id(euid: u32, dev: u64, ino: u64) -> String {
    sha256::hex_digest(format!("session-relay/repository/v1\0{euid}\0{dev}\0{ino}").as_bytes())
}
...
pub fn now_timestamp() -> Result<String, String> {
    ...
    if unsafe { libc::gmtime_r(&time.tv_sec, tm.as_mut_ptr()) }.is_null() {
        return Err("gmtime_r failed".into());
    }
...
pub fn resolve_path_policy(
    ...
    let root = fs::canonicalize(root)
        .map_err(|error| format!("canonicalize claim root {}: {error}", root.display()))?;
    ...
    for claim in explicit_coordinator_paths {
        ...
        if coordinator.insert(key, claim.clone()).is_some() {
            return Err(format!(
                "coordinator-owned path {} is duplicated or case-aliased",
                claim.path
            ));
        }
    }
    ...
    for value in overrides {
        ...
        if object.len() != 2 || !object.contains_key("path") || !object.contains_key("reason") {
            return Err("CoordinatorOwnedOverrideV1 keys differ from the closed schema".into());
        }
        let path = object["path"].as_str()?;
        schema::RelPath::parse(path)?;
        let reason = object["reason"].as_str()?;
        if reason.is_empty() || reason.as_bytes().contains(&0) {
            return Err("coordinator-owned override reason is invalid".into());
        }
        if !allow_overrides {
            return Err("first workspace start cannot override coordinator-owned paths".into());
        }
        let key = path.to_ascii_lowercase();
        let Some(default) = coordinator.get(&key) else {
            return Err(format!(
                "coordinator-owned override {path} does not name an exact resolved default"
            ));
        };
        if default.path != path {
            return Err(format!("coordinator-owned override {path} is a case alias"));
        }
        if !override_paths.insert(key) {
            return Err(format!("coordinator-owned override {path} is duplicated"));
        }
    }
    ...
    for claim in owned_paths {
        ...
        for default in coordinator.values() {
            ...
            if key == default_key
                || key.strip_prefix(&format!("{default_key}/")).is_some()
                || default_key.strip_prefix(&format!("{key}/")).is_some()
            {
                return Err(format!(
                    "owned path {} overlaps coordinator-owned default {}",
                    claim.path, default.path
                ));
            }
        }
    }
...
fn validate_claim_target(root: &Path, claim: &PathClaimRequestV1) -> Result<(), String> {
    ...
    if metadata.file_type().is_symlink() {
        return Err(format!("claim {} is a symlink", claim.path));
    }
    match claim.path_type.as_str() {
        "file" if !metadata.is_file() => {
            Err(format!("file claim {} is not a regular file", claim.path))
        }
        "directory" if !metadata.is_dir() => {
            Err(format!("directory claim {} is not a directory", claim.path))
        }
        _ => Ok(()),
    }
}

fn classify_coordinator_paths(
    root: &Path,
    current: &Path,
    output: &mut BTreeMap<String, PathClaimRequestV1>,
) -> Result<(), String> {
    let mut entries = fs::read_dir(current)
        .map_err(|error| {
            format!(
                "scan coordinator-owned defaults in {}: {error}",
                current.display()
            )
        })?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| format!("scan coordinator-owned default entry: {error}"))?;
    entries.sort_by_key(|entry| entry.file_name());
    for entry in entries {
        let path = entry.path();
        let relative = path
            .strip_prefix(root)
            .map_err(|_| "coordinator classifier path escaped root".to_string())?;
        let relative = relative
            .to_str()
            .ok_or_else(|| "coordinator-owned path is not UTF-8".to_string())?
            .to_string();
        schema::RelPath::parse(&relative)?;
        let metadata = fs::symlink_metadata(&path)
            .map_err(|error| format!("inspect coordinator-owned candidate {relative}: {error}"))?;
        if metadata.file_type().is_symlink() {
            continue;
        }
        ...
        let name = name
            .to_str()
            .ok_or_else(|| "coordinator-owned filename is not UTF-8".to_string())?;
        ...
        if classified {
            let path_type = if metadata.is_dir() {
                ...
                return Err(format!(
                    "coordinator-owned default {relative} has unsupported type"
                ));
            };
            ...
            if output
                ...
                .is_some()
            {
                return Err("coordinator-owned defaults contain case aliases".into());
            }
...
fn coordinator_default(name: &str, relative: &str, is_directory: bool) -> bool {
    ...
    let components = relative
        ...
        .collect::<Vec<_>>();
    if is_directory
        && (matches!(lower.as_str(), ".github" | ".gitlab" | ".circleci")
            || components.iter().any(|component| component == "migrations"))
    {
        return true;
    }
    if is_directory {
    ...
    MANIFESTS.contains(&lower.as_str())
        || LOCKFILES.contains(&lower.as_str())
        || lower.ends_with(".lock")
        || matches!(lower.as_str(), "agents.md" | "claude.md")
...
mod tests {
    use super::*;
    #[test]
    fn repository_id_is_domain_separated() {
        assert_eq!(repository_id(1, 2, 3).len(), 64);
        assert_ne!(repository_id(1, 2, 3), repository_id(1, 2, 4));
    }
    #[test]
    fn timestamp_shape() {
        assert!(schema::Timestamp::parse(&now_timestamp().unwrap()).is_ok());
    }
    #[test]
    fn bootstrap_publish_race_loser_gets_deterministic_guidance() {
        let root = std::env::temp_dir().join(format!(
            "session-relay-bootstrap-{}",
            crate::store::uuid_v4()
        ));
