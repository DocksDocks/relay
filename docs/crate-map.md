...
4. **Custody advances on evidence.** On Linux, the worker is admitted to a cgroup, confined with Landlock, pinned by a pidfd, stopped at exec with `ptrace`, and controlled by two custodians exchanging authenticated packets and kernel FDs (`src/workspace/platform/linux.rs:56-61`, `src/workspace/platform/linux.rs:605-827`, `src/workspace/custody.rs:615-631`). Empty-process, activation, lease-close and crash evidence are persisted before later state becomes valid (`src/workspace/platform/linux.rs:170-200`, `src/workspace.rs:2927-3027`, `src/workspace.rs:5827-6181`). Writable custody is deliberately refused outside the admitted Linux backend (`src/workspace/platform.rs:5-34`, `src/workspace/platform/macos.rs:1-12`).

The source tree is 53,478 lines in 30 files. Inline `#[cfg(test)]` regions account for 4,853 lines; adding the 15,436 lines in `src/tests/` makes 20,289 of 68,914 lines test code (29.4%). The per-file ranges are enumerated below and in §5; representative inline-test boundaries include `src/workspace.rs:8694-9222`, `src/workspace/git.rs:3285-4252`, `src/workspace/schema.rs:3551-3870`, and `src/workspace/platform/linux.rs:2518-2966`.
...
| `src/discover.rs` | 280 | Scans Claude/Codex/omp on-disk session stores without mutating them (`src/discover.rs:13-14`, `src/discover.rs:61-279`). |
...
| `src/workspace/platform/linux.rs` | 2,966 | Implements cgroup/pidfd process custody, runtime closure proof, Landlock and traced activation (`src/workspace/platform/linux.rs:17-61`, `src/workspace/platform/linux.rs:209-2517`). |
...
**Counting rule.** A call site is a syntactic production call expression under `src/`; inline `#[cfg(test)]` modules and `src/tests/` are excluded. When one helper funnels the mechanism, the table gives both raw kernel crossings and wrapper callers.

| Mechanism | What it buys | Primary production use | Call-site count |
|---|---|---|---:|
| cgroup v2 delegated leaf | A kernel-maintained set containing the worker and every descendant; teardown and emptiness do not require PID enumeration. | Opens pinned directory, `events`, `procs` and `kill` FDs after validating a domain leaf and delegated root (`src/workspace/platform/linux.rs:445-510`, `src/workspace/platform/linux.rs:1225-1238`). | One `DelegatedCgroup` implementation; four pinned control FDs (`src/workspace/platform/linux.rs:498-501`). |
| `cgroup.kill` | Atomically queues `SIGKILL` for the whole descendant tree, including processes racing to fork. | Normal fence then empty proof (`src/workspace/platform/linux.rs:748-768`) and failed-launch cleanup (`src/workspace/platform/linux.rs:2220-2285`). | 2 writes: `src/workspace/platform/linux.rs:749`, `src/workspace/platform/linux.rs:2227`. |
| SIGTERM then an independent empty proof | Stops the worker on its own budget, then proves the leaf empty on a second budget, so a slow-to-schedule exit cannot consume the proof. | `graceful_stop_and_wait_empty` (`src/workspace/platform/linux.rs:770-779`) supplies both deadlines to `graceful_stop_and_wait_empty_within` (`src/workspace/platform/linux.rs:794-817`). | 0 raw syscalls; 2 production |
| `cgroup.events` / `populated` | Proves that no process remains in the leaf before evidence or removal is accepted. | Exact `populated 0 or 1` parser `read_populated` (`src/workspace/platform/linux.rs:2135-2147`), the recursive-empty wait (`src/workspace/platform/linux.rs:2116-2133`) and its child-cgroup guard `has_child_cgroups` (`src/workspace/platform/linux.rs:2149-2161`); orphan reconciliation also reads it (`src/workspace/platform/linux.rs:406-444`). | 2 read sites: `src/workspace/platform/linux.rs:407-411`, `src/workspace/platform/linux.rs:2135-2147`. |
| `clone3(CLONE_INTO_CGROUP + CLONE_PIDFD)` | Creates the worker inside the cgroup and returns a pidfd at birth, removing the post-fork migration window. | Worker birth (`src/workspace/platform/linux.rs:1340-1388`), called by launch (`src/workspace/platform/linux.rs:610-667`). | 1 syscall, 1 caller (`src/workspace/platform/linux.rs:624`, `src/workspace/platform/linux.rs:1355`). |
| `pidfd_open` | Pins process identity so PID reuse cannot redirect later observation or signalling. | Admission probe and reusable wrapper (`src/workspace/platform/linux.rs:252-268`, `src/workspace/platform/linux.rs:1037-1059`). | 2 raw syscalls; 4 production wrapper callers (`src/workspace/platform/linux.rs:256`, `src/workspace/platform/linux.rs:667`, `src/workspace/platform/linux.rs:1039`, `src/workspace/custody.rs:381`, `src/workspace.rs:3726`, `src/workspace.rs:4715`). |
| `pidfd_send_signal` | Tests or signals the exact pinned process rather than whatever currently has a numeric PID. | Liveness and real-signal helpers plus custody peer validation (`src/workspace/platform/linux.rs:1061-1122`, `src/workspace/custody.rs:438-470`). | 3 syscalls (`src/workspace/platform/linux.rs:1085`, `src/workspace/platform/linux.rs:1108`, `src/workspace/custody.rs:454`). |
| `waitid(P_PIDFD)` | Observes or reaps the exact process; `WNOWAIT` permits non-consuming liveness checks. | Pidfd liveness, normal reap and failed-launch reap (`src/workspace/platform/linux.rs:1061-1095`, `src/workspace/platform/linux.rs:2200-2218`, `src/workspace/platform/linux.rs:2220-2285`). | 3 calls (`src/workspace/platform/linux.rs:1064`, `src/workspace/platform/linux.rs:2204`, `src/workspace/platform/linux.rs:2246`). |
| Landlock | Applies inherited, unprivileged filesystem rules: runtime files are readable/executable, workspace contents are non-executable, and only admitted paths are writable. | Ruleset construction and restriction before exec (`src/workspace/platform/linux.rs:18-52`, `src/workspace/platform/linux.rs:1905-2037`), invoked in child preparation (`src/workspace/platform/linux.rs:1843-1853`). | 4 syscalls, 1 enforcement caller (`src/workspace/platform/linux.rs:1909`, `src/workspace/platform/linux.rs:1933`, `src/workspace/platform/linux.rs:1962`, `src/workspace/platform/linux.rs:2022`). |
...
| `ptrace(TRACEME/SETOPTIONS/DETACH)` | Stops the worker at exec, arms `PTRACE_O_EXITKILL`, verifies identity/membership/executable before release, and kills it if activation fails. | Child exec barrier, parent trap check, clean detach and failure detach (`src/workspace/platform/linux.rs:1390-1422`, `src/workspace/platform/linux.rs:1830-1904`, `src/workspace/platform/linux.rs:916-966`). | 4 production calls: one TRACEME, one SETOPTIONS, two DETACH (`src/workspace/platform/linux.rs:929`, `src/workspace/platform/linux.rs:951`, `src/workspace/platform/linux.rs:1409`, `src/workspace/platform/linux.rs:1886`). |
...
| fd-relative `openat` + `O_NOFOLLOW` + identity revalidation | Anchors each lookup to an already-open directory, refuses a final symlink, then closes TOCTOU by comparing device/inode and mount identity before use. | Repository marker admission (`src/workspace/repository_gate.rs:92-312`, `src/workspace/repository_gate.rs:621-685`) and common helpers (`src/workspace/platform/linux.rs:2297-2328`, `src/workspace/platform/linux.rs:2463-2495`). | 16 `openat` expressions; 57 production `NOFOLLOW` sites; 6 fd mount-ID and 2 path mount-ID checks (`src/fanout.rs:285`, `src/store.rs:858-1177`, `src/fanout/git.rs:158`, `src/workspace/git.rs:730-757`, `src/workspace.rs:1810-1857`, `src/workspace/repository_gate.rs:240-280`, `src/workspace/platform/linux.rs:2315`, `src/workspace/platform/linux.rs:210-213`, `src/workspace/platform/linux.rs:372-398`, `src/workspace/repository_gate.rs:638`, `src/workspace.rs:1019-1026`). |

**Correction: there is no production `openat2(RESOLVE_NO_SYMLINKS)`.** The executable occurrence is only a dormant census regex (`test/reentry-inventory.mjs:160`); it currently matches no source call. The `openat2(2)` manual is a design reference for this crate, not an implementation claim. The implemented defence is the `openat`/`O_NOFOLLOW`/identity chain in the final table row.

## 5. Test topology

The repository gate invokes fourteen Rust test-inventory cases. Thirteen cases each run one integration-test binary; the `unit` case runs `cargo test --locked --lib`. Each integration case runs its binary serially, compares live `cargo test --list` names with a frozen fixture, and requires zero ignored or filtered tests (`test/rust-test-inventory.mjs`).

The table below covers the eight integration targets whose warm run durations this reading measured. The frozen counts live in one fixture, which the harness regenerates, so the cells cite the fixture without line ranges.

| Gate case | Test file and size | Frozen tests | Warm run |
|---|---|---:|---:|
| `protocol` | `src/tests/protocol.rs`, 2,074 lines (`src/tests/protocol.rs:1-2074`) | 38 (`test/fixtures/rust-test-inventory.json`) | 3.3 s |
| `fanout` | `src/tests/fanout.rs`, 2,592 lines (`src/tests/fanout.rs:1-2592`) | 37 (`test/fixtures/rust-test-inventory.json`) | 17.2 s |
| `fanout_reap` | `src/tests/fanout_reap.rs`, 520 lines (`src/tests/fanout_reap.rs:1-520`) | 11 (`test/fixtures/rust-test-inventory.json`) | 2.0 s |
| `lifecycle_supervisor` | `src/tests/lifecycle_supervisor.rs`, 1,039 lines (`src/tests/lifecycle_supervisor.rs:1-1039`) | 13 (`test/fixtures/rust-test-inventory.json`) | 10.9 s |
| `workspace_identity` | `src/tests/workspace_identity.rs`, 1,388 lines (`src/tests/workspace_identity.rs:1-1388`) | 10 (`test/fixtures/rust-test-inventory.json`) | 48.6 s |
| `workspace_lease_process` | `src/tests/workspace_lease_process.rs`, 1,898 lines (`src/tests/workspace_lease_process.rs:1-1898`) | 9 (`test/fixtures/rust-test-inventory.json`) | 49.8 s |
| `workspace_coordination_process` | `src/tests/workspace_coordination_process.rs`, 1,458 lines (`src/tests/workspace_coordination_process.rs:1-1458`) | 9 (`test/fixtures/rust-test-inventory.json`) | 248.7 s |
| `workspace_resources` | `src/tests/workspace_resources.rs`, 405 lines (`src/tests/workspace_resources.rs:1-405`) | 3 (`test/fixtures/rust-test-inventory.json`) | 6.7 s |

The unrounded warm run-only durations totalled 387.1 s on a quiet 6-vCPU Linux host; the independently rounded table cells sum to 387.2 s. The run used the harness's serial command shape and one delegated systemd scope per target (`test/rust-test-inventory.mjs`). `workspace_coordination_process` consumed 64.2%; `workspace_identity`, `workspace_lease_process` and `workspace_coordination_process` together consumed 89.7%, and all four `workspace_*` targets consumed 91.4%. A warm incremental `cargo build --tests --locked` took another 12.2 s; build time is not included in the table. These are local observations, not CI budgets. The source explains the distribution: coordination repeatedly launches custody and fault-replay scenarios, identity and lease tests perform real Git/process work, resources has a hard 4–8 s timeout case, and lifecycle supervision creates real relay processes with at least 1.2 s of explicit observation waits (`src/tests/workspace_coordination_process.rs:302-1458`, `src/tests/workspace_identity.rs:151-1165`, `src/tests/workspace_lease_process.rs:349-1782`, `src/tests/workspace_resources.rs:363-382`, `src/tests/lifecycle_supervisor.rs:491-605`).

No integration target stays outside the gate. The five binaries that earlier waivers excluded—`bus_smoke`, `lifecycle_admission`, `lifecycle_managed`, `lifecycle_release` and `lock_race`—now own gate cases of their own (`test/rust-test-inventory.mjs`). The Rust gate formats, lints and builds, and then runs all fourteen cases. The `unit` case runs `cargo test --locked --lib`, so the gate also executes the inline `#[cfg(test)]` library tests.

## 6. Comprehension hazards

### Invariants and ordering that are easy to miss

- **PID identity is `(pid, starttime)`, not PID alone.** `/proc/<pid>/stat` field 22 is parsed after the last `) ` because the parenthesized `comm` field can contain spaces and parentheses; received pidfds are then cross-checked against both identities and liveness (`src/workspace/platform/linux.rs:967-1035`).
- **BOOTSTRAP FD order is an ABI.** The positional array is `[lease, cgroup-directory, cgroup.events, cgroup.procs]`; sender and receiver destructure exactly that order, and only count/type/target checks protect it (`src/workspace/platform/linux.rs:520-591`, `src/workspace/platform/linux.rs:1149-1223`).
- **The worker activation order is security-critical.** The child sets parent-death signal and no-new-privileges, applies Landlock, closes all unlisted FDs, changes directory, publishes “prepared,” waits for activation, requests tracing and execs; the parent proves cgroup membership/starttime, waits for the exec trap, arms `EXITKILL`, verifies the executable and only then detaches (`src/workspace/platform/linux.rs:610-745`, `src/workspace/platform/linux.rs:829-966`, `src/workspace/platform/linux.rs:1390-1422`, `src/workspace/platform/linux.rs:1830-1904`).
...
- The payload context says “four exact Rust target inventories,” but the repository gate runs fourteen `rust-test-inventory.mjs --case …` cases (`plugin/AGENTS.md`). The same sentence forbids hidden platform skips, while workspace tests return successfully from native-only branches (`plugin/AGENTS.md`, `src/tests/workspace_lease_process.rs:349-356`, `src/tests/workspace_coordination_process.rs:302-306`).
- Passing-count equality does not prove behavior ran: workspace tests use successful early returns off Linux, and one cgroup reconciliation test returns on Linux when `SESSION_RELAY_TEST_CGROUP_ROOT` is absent (`src/tests/workspace_lease_process.rs:1518-1523`, `src/tests/workspace_coordination_process.rs:302-306`, `src/tests/workspace_coordination_process.rs:640-643`). Read a green count together with the harness delegation and platform path (`test/rust-test-inventory.mjs`).
- `workspace_resources` is the only selected target with a hard elapsed-time assertion; `elapsed < 8s` can fail under load even when the provider is correctly fenced after its configured 5 s timeout (`src/tests/workspace_resources.rs:363-382`, `src/workspace/resources.rs:25-29`).
- The cgroup object pins and transfers a long-lived `cgroup.events` FD, but `wait_recursive_empty` ignores it, reopens by path through `read_populated`, and sleeps 20 ms between reads (`src/workspace/platform/linux.rs:498-525`, `src/workspace/platform/linux.rs:565-600`, `src/workspace/platform/linux.rs:2116-2133`). On cgroup v2, the useful readiness notification is `POLLPRI`; polling for ordinary `POLLIN` would be an always-ready no-op, so any future event-driven rewrite must keep one FD for read → poll → re-read rather than merely changing the sleep (`src/workspace/platform/linux.rs:499`, `src/workspace/platform/linux.rs:2116-2133`).
- `clone3` is a preferred path, not the only path. Selected compatibility errors fall back to `fork`, then write the PID to `cgroup.procs` and open a pidfd; reasoning that assumes the child is always born in the cgroup is incomplete (`src/workspace/platform/linux.rs:624-689`, `src/workspace/platform/linux.rs:1346-1388`).
- This blind spot is now closed: `bus_smoke.rs`, `lock_race.rs` and the three lifecycle integration binaries own gate cases, so the ordinary Rust source checks exercise the code that only they reach (`test/rust-test-inventory.mjs`).
