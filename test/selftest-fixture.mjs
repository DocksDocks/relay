...
export function createFixture({ bin: configuredBin, home }) {
  ...
  async function terminate(record) {
    ...
    const groupTargets = processGroup && Number.isInteger(child.pid) ? [processGroupTarget(child.pid)] : [];
    const targets = [...groupTargets, ...(childClosed ? [] : processTreeTargets(child.pid))];
    ...
    const survivors = liveTargets(remaining);
    if (survivors.length > 0)
      throw new Error(`tracked child process tree survived SIGKILL: ${survivors.map((s) => s.pid).join(', ')}`);
