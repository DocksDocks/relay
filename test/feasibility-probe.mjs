...
if (process.env.RELAY_REAL_RUNTIME_TEST !== '1' || process.argv.length !== 3 || process.argv[2] !== '--verify-current') {
  console.error('usage: RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --verify-current');
  process.exit(2);
}
...
}).map(([key, value]) => { mkdir0700(value); return [key, value]; }));
...
function isolatedEnv(extra = {}) {
  const env = {};
  for (const key of ['PATH', 'SHELL', 'LANG', 'LC_ALL', 'LC_CTYPE', 'TERM', 'HTTP_PROXY', 'HTTPS_PROXY', 'ALL_PROXY', 'NO_PROXY', 'http_proxy', 'https_proxy', 'all_proxy', 'no_proxy', 'SSL_CERT_FILE', 'SSL_CERT_DIR', 'NODE_EXTRA_CA_CERTS', 'CODEX_CA_CERTIFICATE', 'ANTHROPIC_BASE_URL']) {
...
async function runProcess(argv, { cwd = dirs.cwd, env = isolatedEnv(), timeoutMs = 120_000, input = null } = {}) {
  ...
  return await new Promise((resolve) => {
    ...
    try {
      ...
      resolve({ startedAt, finishedAt, durationMs: 0, status: null, signal: null, timedOut: false, stdout: Buffer.alloc(0), stderr: Buffer.from(String(error), 'utf8') });
      return;
    }
    const stdout = [];
    const stderr = [];
    ...
    child.on('close', (status, signal) => {
      ...
      resolve({ startedAt, finishedAt: isoNow(), durationMs, status, signal, timedOut, stdout: Buffer.concat(stdout), stderr: Buffer.concat(stderr) });
    });
...
function syntheticResult(raw, { status = 0, stderr = '', durationMs = 0 } = {}) {
  const now = isoNow();
  return { startedAt: now, finishedAt: now, durationMs, status, signal: null, timedOut: false, stdout: buffer(raw), stderr: buffer(stderr) };
}
...
parsers.set('command-version', (record, stdout, stderr) => {
  ...
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'runtime version recorded' : `version command failed: ${firstLine(stderr) || 'no version output'}`, facts: { version } };
});
parsers.set('command-available', (record, stdout, stderr) => {
  const ok = record.exit_status === 0 && !record.timed_out;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'command completed' : `command unavailable: ${firstLine(stderr) || `status=${record.exit_status}`}`, facts: { stdout_sha256: sha256(stdout), stderr_sha256: sha256(stderr) } };
});
parsers.set('auth-preflight', (record, stdout, stderr) => {
  let envelope;
  try { envelope = JSON.parse(stdout.toString('utf8')); } catch { envelope = {}; }
  const runtimeOut = Buffer.from(envelope.runtime_stdout_base64 || '', 'base64').toString('utf8').trim();
  const ok = record.exit_status === 0 && !record.timed_out && runtimeOut === 'RELAY_AUTH_OK';
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'isolated authenticated preflight returned exact marker' : envelope.unavailable_reason || firstLine(Buffer.from(envelope.runtime_stderr_base64 || '', 'base64')) || firstLine(stderr) || 'authentication preflight did not return the exact marker', facts: { marker_exact: runtimeOut === 'RELAY_AUTH_OK', auth_source: envelope.auth_source || 'unavailable' } };
});
parsers.set('hook-contract', (record, stdout) => {
  ...
  const logRows = hookLog.split(/\r?\n/).filter(Boolean).flatMap((line) => { try { return [JSON.parse(line)]; } catch { return []; } });
  const targetRows = logRows.filter((row) => row.event === rule.event && row.phase === 'start');
  const loadedExact = targetRows.some((row) => row.hook_path === envelope.hook_path && row.hook_sha256 === envelope.hook_sha256);
  ...
  return {
    ...
    reason: ok ? `${rule.event} ${rule.mode} behavior matched raw runtime and hook evidence` : envelope.unavailable_reason || 'hook behavior, load identity, or runtime marker did not match',
    facts: { event: rule.event, mode: rule.mode, hook_loaded: loadedExact, hook_path: envelope.hook_path || null, hook_sha256: envelope.hook_sha256 || null, marker_present: markerPresent, expected_marker: expectedMarker, hook_invocations: targetRows.length },
  };
});
parsers.set('attach-timing', (record, stdout) => {
  ...
  const attachMs = end && Number.isFinite(envelope.launched_at_ms) ? end.observed_at_ms - envelope.launched_at_ms : null;
  const ok = record.exit_status === 0 && !record.timed_out && runtimeOut.includes(marker) && Number.isInteger(attachMs) && attachMs >= 0 && envelope.lock_contention === true;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'cold SessionStart attach completed with real relay hook and sequential store-lock contention' : envelope.unavailable_reason || 'timing sample lacked marker, delegated hook end timestamp, or lock contention', facts: { sample: envelope.sample ?? null, measured_ms: attachMs, process_duration_ms: record.duration_ms, lock_contention: envelope.lock_contention === true, marker_present: runtimeOut.includes(marker) } };
});
parsers.set('json-facts', (record, stdout) => {
  let raw;
  try { raw = JSON.parse(stdout.toString('utf8')); } catch { raw = { availability: 'unavailable', reason: 'raw JSON parse failed', facts: {} }; }
  const availability = ['unavailable', 'observation_only', 'not_applicable'].includes(raw.availability) ? raw.availability : 'unavailable';
  return { availability, reason: String(raw.reason || 'raw probe supplied no reason'), facts: raw.facts && typeof raw.facts === 'object' && !Array.isArray(raw.facts) ? raw.facts : {} };
});
parsers.set('cgroup-v2', (_record, stdout) => {
  let raw; try { raw = JSON.parse(stdout.toString('utf8')); } catch { raw = { facts: {} }; }
  const facts = raw.facts || {};
  const ok = facts.linux === true && facts.delegation_create === true && facts.delegation_move === true && facts.cgroup_kill_present === true && facts.cgroup_freeze_present === true && facts.freeze_observed === true && facts.kill_observed === true && facts.populated_zero === true && Array.isArray(facts.events) && facts.events.some((event) => event.op === 'move_child') && facts.events.some((event) => event.op === 'freeze') && facts.events.some((event) => event.op === 'kill');
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'delegated cgroup v2 leaf passed create/move/freeze/kill/populated-zero probes' : String(raw.reason || 'one or more delegated cgroup v2 operations is unavailable'), facts };
});
parsers.set('namespace-isolation', (record, stdout) => {
  let raw; try { raw = JSON.parse(stdout.toString('utf8')); } catch { raw = { facts: {} }; }
  const facts = raw.facts || {};
  let helper = {};
  try { helper = JSON.parse(Buffer.from(facts.helper_stdout_base64 || '', 'base64').toString('utf8')); } catch {}
  const ok = record.exit_status === 0 && helper.user_namespace === true && helper.mount_pid_cgroup_namespaces === true && helper.namespace_pid_is_one === true && helper.inherited_proc_cgroup_mounts === helper.detached_proc_cgroup_mounts && helper.fresh_proc === true && helper.fresh_cgroup2 === true && helper.cgroup2_read_only === true && helper.proc_mount_count === 1 && helper.cgroup2_mount_count === 1 && helper.proc_pid_fd_authority_denied === true;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'unprivileged user plus mount/PID/cgroup namespaces, detach-all, fresh proc/cgroup views, and proc-pid-fd denial succeeded' : String(raw.reason || 'namespace isolation prerequisite failed'), facts };
});
parsers.set('seccomp-description', (record, stdout) => {
  let raw; try { raw = JSON.parse(stdout.toString('utf8')); } catch { raw = { facts: {} }; }
  const facts = raw.facts || {};
  let description = {};
  try { description = JSON.parse(Buffer.from(facts.raw_stdout_base64 || '', 'base64').toString('utf8')); } catch {}
  const expectedArch = process.arch === 'x64' ? 'AUDIT_ARCH_X86_64' : process.arch === 'arm64' ? 'AUDIT_ARCH_AARCH64' : 'unsupported';
  const ok = record.exit_status === 0 && description.audit_arch === expectedArch && (process.arch !== 'x64' || description.x32_rejection === true) && Number.isInteger(description.instructions) && description.instructions > 0;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'compiled filter reports the exact native audit architecture and x32 branch' : 'filter description lacks the required architecture/x32 structure', facts };
});
parsers.set('x32-probe', (record) => {
  const ok = process.arch === 'x64' ? record.signal === 'SIGSYS' && !record.timed_out : true;
  return { availability: process.arch === 'x64' ? (ok ? 'available' : 'unavailable') : 'not_applicable', reason: process.arch === 'x64' ? (ok ? 'x32 syscall-number probe was killed by the architecture gate' : 'x32 syscall-number probe was not killed with SIGSYS') : 'x32 ABI exists only on x86_64', facts: { arch: process.arch, status: record.exit_status, signal: record.signal, timed_out: record.timed_out } };
});
parsers.set('native-pidfd', (record, stdout, stderr) => {
  let raw = {}; try { raw = JSON.parse(stdout.toString('utf8')); } catch {}
  const ok = record.exit_status === 0 && raw.pidfd_open === true && raw.proc_pid_fd === true;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'native pidfd_open pinned the current process and proc-pid-fd is present' : firstLine(stderr) || 'native pidfd/proc-pid-fd probe is unavailable', facts: { pidfd_open: raw.pidfd_open ?? false, pidfd_errno: raw.pidfd_errno ?? null, proc_pid_fd: raw.proc_pid_fd ?? false } };
});
parsers.set('seccomp-filter', (record, stdout) => {
  ...
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'compiled helper emitted the exact candidate BPF bytes' : 'candidate BPF dump failed or changed', facts: { bpf_sha256: actual, bpf_bytes: stdout.length, policy_sha256: rule.policy_sha256, helper_sha256: rule.helper_sha256, audit_arch: rule.audit_arch, x32_rejection: rule.x32_rejection, clone3_errno: 'ENOSYS' } };
});
parsers.set('seccomp-raw', (record, stdout, stderr) => {
  let raw;
  try { raw = JSON.parse(stdout.toString('utf8')); } catch { raw = {}; }
  const ok = record.exit_status === 0 && !record.timed_out && raw.clone3_rc === -1 && raw.clone3_errno === 38 && raw.clone3_child_created === false && raw.legacy_namespace_clone_denied === true;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'raw clone3 returned ENOSYS without a child and legacy namespace clone was denied' : firstLine(stderr) || 'raw namespace denial evidence did not match', facts: { clone3_rc: raw.clone3_rc ?? null, clone3_errno: raw.clone3_errno ?? null, clone3_child_created: raw.clone3_child_created ?? null, legacy_namespace_clone_denied: raw.legacy_namespace_clone_denied ?? null, legacy_clone_errno: raw.legacy_clone_errno ?? null } };
});
parsers.set('runtime-spawn', (record, stdout) => {
  ...
  const ok = record.exit_status === 0 && !record.timed_out && runtimeOut.includes('RELAY_STRONG_SPAWN_OK') && lines.length === 2 && lines[0] === 'child-complete' && lines[1] === 'wait-complete' && envelope.child_reaped === true;
  return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'real runtime created an ordinary tool child, observed descendant completion, and waited under the candidate filter' : envelope.unavailable_reason || 'no safe legacy-clone fallback or missing child/wait sentinel', facts: { child_complete: lines[0] === 'child-complete', wait_complete: lines[1] === 'wait-complete', ordered: lines.join(',') === 'child-complete,wait-complete', child_reaped: envelope.child_reaped === true, filter_sha256: envelope.filter_sha256 || null, policy_sha256: envelope.policy_sha256 || null, clone3_errno: 'ENOSYS' } };
});
parsers.set('appserver-protocol', (record, stdout) => {
  ...
  for (const entry of raw.schema_manifest || []) {
    ...
    assert.equal(absolute.startsWith(`${path.resolve(dirs.appserverSchema)}${path.sep}`), true, 'app-server schema artifact stays inside the isolated schema directory');
  ...
  assert.equal(sha256(Buffer.from(canonical(raw.schema_manifest || []))), raw.schema_bundle_sha256, 'app-server schema bundle hash');
  ...
  if ((raw.responses || []).some((response) => graceful.has(response.method) && response.reason === 'graceful_exit' && response.method_succeeded && response.exited_before_signal && response.reaped) && contract.shutdown_ack.length > 0) contract.reap.push('supervisor_child_reaped_after_graceful_shutdown_ack');
  ...
  const complete = ['mutation_barrier', 'graceful_stop', 'mutation_reject', 'durable_flush', 'accepted_watermark', 'flushed_watermark', 'shutdown_ack', 'storage_sync', 'reap'].every((key) => Array.isArray(contract[key]) && contract[key].length > 0);
  return {
    availability: complete ? 'available' : 'unavailable',
    reason: complete ? 'schema and raw responses expose the complete stop/reject/flush/watermark/reap contract' : raw.reason || 'protocol_tree=unavailable: no complete durable graceful-flush contract',
    facts: { protocol_tree: complete ? 'available' : 'unavailable', shared_protocol: 'observation_only', deadline_ms: PROTOCOL_DEADLINE_MS, measured_ms: raw.measured_ms ?? record.duration_ms, schema_bundle_sha256: raw.schema_bundle_sha256 || null, methods: raw.methods || [], contract, responses: raw.responses || [], process_reaped: raw.process_reaped === true },
  };
});
...
function streamDescriptor(id, streamName, bytes) {
  ...
  return { artifact, sha256: sha256(data), bytes: data.length, base64: data.length <= INLINE_STREAM_LIMIT ? data.toString('base64') : null };
}

function addRecord({ id, category, runtime, runtimeVersion = null, argv, result, parserId, parserRule }) {
  assert.equal(records.some((record) => record.id === id), false, `duplicate record id ${id}`);
  const record = {
    id,
    category,
    runtime,
...
function addUnavailable({ id, category, runtime, runtimeVersion = null, argv, reason, facts = {} }) {
  return addRecord({
    id, category, runtime, runtimeVersion, argv,
    result: syntheticResult(jsonBuffer({ availability: 'unavailable', reason, facts })),
    parserId: 'json-facts', parserRule: 'Use raw availability/reason/facts; unavailability is a valid fail-closed observation.',
  });
}
...
function readRecordStream(record, name) {
  ...
  if (descriptor.base64 !== null) assert.equal(Buffer.from(descriptor.base64, 'base64').equals(bytes), true, `${record.id} ${name} base64`);
  return bytes;
}
...
const SECCOMP_POLICY = {
  ...
  legacy_clone_namespace_flags: { action: 'errno', errno: 'EPERM', flags: ['CLONE_NEWCGROUP', 'CLONE_NEWIPC', 'CLONE_NEWNET', 'CLONE_NEWNS', 'CLONE_NEWPID', 'CLONE_NEWUSER', 'CLONE_NEWUTS'] },
  errno_eperm_syscalls: ['mount', 'umount2', 'fsopen', 'fsmount', 'open_tree', 'move_mount', 'mount_setattr', 'fsconfig', 'fspick', 'pivot_root', 'chroot', 'setns', 'unshare', 'ptrace', 'process_vm_writev', 'pidfd_getfd'],
};
...
async function compileHelper(id, category, source, output) {
  if (process.platform !== 'linux') {
    addUnavailable({ id, category, runtime: 'host', argv: ['cc', source, '-o', output], reason: 'Linux-only helper is not applicable on this host', facts: { source_sha256: sha256(fs.readFileSync(source)) } });
    return false;
  }
  const argv = ['cc', '-O2', '-Wall', '-Wextra', '-Werror', source, '-o', output];
  const result = await runProcess(argv, { timeoutMs: 30_000 });
  const record = addRecord({ id, category, runtime: 'host', argv, result, parserId: 'command-available', parserRule: 'Exit zero proves the host compiler accepted the committed helper source.' });
...
async function runRuntime(runtime, prompt, options = {}) {
  if (auth[runtime].kind === 'unavailable') return { unavailableReason: auth[runtime].reason, command: runtimeCommand(runtime, prompt, options) };
...
for (const runtime of RUNTIMES) {
  ...
  const record = addRecord({ id: `runtime.${runtime}.version`, category: 'runtime_version', runtime, argv, result, parserId: 'command-version', parserRule: 'Exit zero and the first non-empty stdout line are the installed runtime version.' });
...
for (const runtime of RUNTIMES) {
  ...
  if (run.unavailableReason) {
    result = syntheticResult(jsonBuffer({ auth_source: 'unavailable', unavailable_reason: run.unavailableReason, runtime_stdout_base64: '', runtime_stderr_base64: '' }));
  } else {
    result = { ...run.result, stdout: jsonBuffer({ auth_source: run.command.authSource, runtime_stdout_base64: run.result.stdout.toString('base64'), runtime_stderr_base64: run.result.stderr.toString('base64') }), stderr: Buffer.alloc(0) };
  }
  const record = addRecord({ id: `runtime.${runtime}.auth`, category: 'auth_preflight', runtime, runtimeVersion: versions[runtime], argv: run.command.logical, result, parserId: 'auth-preflight', parserRule: 'The isolated CLI must exit zero and stdout, after trimming, must equal RELAY_AUTH_OK.' });
...
async function hookContractRow(runtime, event, mode) {
  ...
  return addRecord({
    ...
    category: 'hook_contract', runtime, runtimeVersion: versions[runtime], argv: command.logical, result,
    parserId: 'hook-contract', parserRule: canonical({ event, mode, marker }),
  });
}
...
async function startStoreContention(sample) {
  ...
  let child;
  try { child = spawn('flock', ['-x', lock, 'sh', '-c', script], { cwd: dirs.cwd, env: isolatedEnv(), stdio: 'ignore' }); }
  catch { return { available: false, reason: 'flock command could not start', wait: Promise.resolve() }; }
  const deadline = Date.now() + 3000;
  while (!exists(ready) && Date.now() < deadline && child.exitCode === null) await sleep(10);
  const available = exists(ready);
  return { available, reason: available ? null : 'flock did not acquire the isolated store lock', wait: new Promise((resolve) => child.once('close', resolve)) };
}
...
async function timingRow(runtime, sample) {
  ...
  if (!authAvailable[runtime] || !exists(relayBin)) {
    unavailableReason = !authAvailable[runtime] ? 'isolated runtime authentication preflight failed' : 'committed relay launcher is unavailable';
  ...
  return addRecord({ id: `timing.${runtime}.${sample}`, category: 'attach_timing', runtime, runtimeVersion: versions[runtime], argv: command.logical, result: { ...runtimeResult, stdout: jsonBuffer(envelope), stderr: Buffer.alloc(0) }, parserId: 'attach-timing', parserRule: 'Require the unique runtime marker, delegated real relay hook evidence, and acquired isolated store-lock contention.' });
}
...
function collectSchemaStrings(value, out = []) {
  if (typeof value === 'string') out.push(value);
    ...
    for (const [key, child] of Object.entries(value)) { out.push(key); collectSchemaStrings(child, out); }
  }
  return out;
}
...
function classifyProtocol(schemaFiles) {
  ...
  const contract = {
    ...
    graceful_stop: matching(/graceful.*(?:stop|shutdown)|(?:server|app).*shutdown|shutdown.*(?:server|app)/i, methodList),
...
async function appserverRequest(method, deadlineAt, expectGracefulExit = false) {
  ...
  return await new Promise((resolve) => {
    ...
    child.stderr.on('data', (chunk) => stderr.push(Buffer.from(chunk)));
    child.stdin.on('error', (error) => stderr.push(Buffer.from(`stdin error: ${error}\n`, 'utf8')));
    child.on('error', (error) => { stderr.push(Buffer.from(String(error))); clearTimeout(timer); finish('spawn_error'); });
    child.on('close', (status, signal) => {
      if (!resolved) finish(expectGracefulExit && methodSucceeded ? 'graceful_exit' : 'unexpected_exit', false);
      resolve({ method, reason: finishReason, status, signal, method_succeeded: methodSucceeded, exited_before_signal: !signaledByHarness, transcript: lines, stderr_base64: Buffer.concat(stderr).toString('base64'), measured_ms: Number((process.hrtime.bigint() - started) / 1_000_000n), reaped: true });
    });
    ...
    child.stdout.on('data', (chunk) => {
      ...
      while ((newline = pending.indexOf('\n')) >= 0) {
        ...
        let message;
        try { message = JSON.parse(line); } catch { message = { raw_base64: Buffer.from(line).toString('base64') }; }
    ...
    const initialize = { id: 0, method: 'initialize', params: { clientInfo: { name: 'relay-feasibility-probe', title: 'relay feasibility probe', version: '1' } } };
...
const schemaManifest = schemaFiles.map((file) => {
  ...
  return { path: relative, artifact: path.posix.join('appserver-schema', relative.split(path.sep).join('/')), bytes: fs.statSync(file).size, sha256: sha256(fs.readFileSync(file)) };
});
...
const probeMethods = [...new Set(Object.values(classified.contract).flat())].filter((method) => classified.methods.includes(method)).slice(0, 12);
const protocolDeadlineAt = Date.now() + Math.max(0, PROTOCOL_DEADLINE_MS - Number((process.hrtime.bigint() - appserverStarted) / 1_000_000n));
const responses = [];
...
const gracefulReaped = responses.some((response) => gracefulMethods.has(response.method) && response.reason === 'graceful_exit' && response.method_succeeded && response.exited_before_signal && response.reaped);
if (gracefulReaped && classified.contract.shutdown_ack.length > 0) classified.contract.reap.push('supervisor_child_reaped_after_graceful_shutdown_ack');
...
const appserverRaw = {
  ...
  reason: schemaResult.status === 0 ? 'protocol_tree=unavailable unless every graceful stop/reject/flush/watermark/ack/sync/reap element is present in raw schema and responses' : 'protocol_tree=unavailable: app-server schema export failed',
};
addRecord({
  id: 'appserver.protocol.contract', category: 'appserver_protocol', runtime: 'app-server', runtimeVersion: versions.codex,
  argv: schemaArgv, result: { ...schemaResult, durationMs: appserverMeasuredMs, stdout: jsonBuffer(appserverRaw), stderr: Buffer.alloc(0) },
  parserId: 'appserver-protocol', parserRule: 'A protocol-tree tier exists only when raw schema/responses contain every graceful mutation barrier, stop, reject, durable flush, accepted/flushed watermark, shutdown ack, storage sync, and supervisor reap element within one 20s deadline.',
});
...
async function probeCgroupV2() {
  ...
  try {
    ...
    const membership = fs.readFileSync('/proc/self/cgroup', 'utf8').split('\n').find((line) => line.startsWith('0::'));
    facts.events.push({ op: 'read_mountinfo', sha256: sha256(Buffer.from(mountinfo)), cgroup2_row: cgroupLine || null });
    facts.events.push({ op: 'read_self_cgroup', raw: membership || null });
    if (!cgroupLine || !membership) return { availability: 'unavailable', reason: 'unified cgroup v2 mount or membership row is absent', facts };
    ...
    if (facts.cgroup_freeze_present) {
      ...
      while (Date.now() < freezeDeadline) {
        ...
        if (/^frozen 1$/m.test(events)) { facts.freeze_observed = true; facts.events.push({ op: 'freeze', events: events.trim() }); break; }
        await sleep(10);
      }
    ...
    const ok = facts.delegation_create && facts.delegation_move && facts.cgroup_kill_present && facts.cgroup_freeze_present && facts.freeze_observed && facts.kill_observed && facts.populated_zero;
    return { availability: ok ? 'available' : 'unavailable', reason: ok ? 'delegated cgroup v2 leaf passed create/move/freeze/kill/populated-zero probes' : 'one or more delegated cgroup v2 operations is unavailable', facts };
  } catch (error) {
    facts.events.push({ op: 'error', code: error.code || null, message: String(error.message || error) });
    return { availability: 'unavailable', reason: `cgroup v2 delegation probe failed: ${error.code || error.message || error}`, facts };
  } finally {
    if (child && child.exitCode === null) {
      try { child.kill('SIGKILL'); } catch {}
      await new Promise((resolve) => child.once('close', resolve));
    }
    if (leaf) { try { fs.rmdirSync(leaf); } catch {} }
  }
}

const cgroupResult = await probeCgroupV2();
addRecord({ id: 'host.cgroup-v2', category: 'cgroup_v2', runtime: 'host', argv: ['node:fs', '/proc/self/mountinfo', '/proc/self/cgroup', '<delegated-leaf>'], result: syntheticResult(jsonBuffer(cgroupResult)), parserId: 'cgroup-v2', parserRule: 'Recompute availability from raw leaf creation, child migration, frozen=1, cgroup.kill, populated=0, and matching operation events; absent delegation is valid unavailability.' });

let namespaceRecord;
if (!namespaceHelperAvailable) {
  namespaceRecord = addUnavailable({ id: 'host.namespace-isolation', category: 'namespace_isolation', runtime: 'host', argv: [helperPaths.namespace], reason: 'namespace helper did not compile', facts: {} });
} else {
  const rawResult = await runProcess([helperPaths.namespace], { env: isolatedEnv({ RELAY_PROBE_HOST_PID: String(process.pid) }), timeoutMs: 15_000 });
  let facts = {};
...
addRecord({ id: 'host.native-process-and-runners', category: 'native_process', runtime: 'host', argv: ['node:fs', '/proc/self/stat', '.github/workflows/build-binaries.yml'], result: syntheticResult(jsonBuffer({ availability: 'observation_only', reason: 'native process generation fields and current workflow runner labels are observations, never stable signal handles', facts: nativeFacts })), parserId: 'json-facts', parserRule: 'Record the native host process observation and exact workflow runner/target rows without promoting either to signal authority.' });

if (seccompHelperAvailable) {
  const nativePidfdResult = await runProcess([helperPaths.seccomp, '--native-probe'], { timeoutMs: 10_000 });
  addRecord({ id: 'host.native-pidfd', category: 'native_process', runtime: 'host', argv: [helperPaths.seccomp, '--native-probe'], result: nativePidfdResult, parserId: 'native-pidfd', parserRule: 'Derive Linux stable-handle availability from an actual pidfd_open on the current process plus proc-pid-fd presence.' });
} else {
  addUnavailable({ id: 'host.native-pidfd', category: 'native_process', runtime: 'host', argv: [helperPaths.seccomp, '--native-probe'], reason: process.platform === 'linux' ? 'native pidfd helper did not compile' : 'pidfd is Linux-only', facts: {} });
}
...
if (!seccompHelperAvailable) {
  addUnavailable({ id: 'host.seccomp-filter', category: 'seccomp_filter', runtime: 'host', argv: [helperPaths.seccomp, '--dump-bpf'], reason: 'candidate seccomp helper did not compile', facts: { policy_sha256: policySha, helper_sha256: seccompHelperSha } });
  addUnavailable({ id: 'host.seccomp-raw', category: 'seccomp_raw_probe', runtime: 'host', argv: [helperPaths.seccomp, '--raw-probe'], reason: 'candidate seccomp helper did not compile', facts: {} });
  addUnavailable({ id: 'host.seccomp-x32', category: 'seccomp_raw_probe', runtime: 'host', argv: [helperPaths.seccomp, '--probe-x32'], reason: 'candidate seccomp helper did not compile', facts: {} });
} else {
  const describeResult = await runProcess([helperPaths.seccomp, '--describe'], { timeoutMs: 10_000 });
  let description = {};
  try { description = JSON.parse(describeResult.stdout.toString('utf8')); } catch {}
  addRecord({ id: 'host.seccomp-description', category: 'seccomp_filter', runtime: 'host', argv: [helperPaths.seccomp, '--describe'], result: { ...describeResult, stdout: jsonBuffer({ availability: describeResult.status === 0 ? 'available' : 'unavailable', reason: describeResult.status === 0 ? 'compiled filter reports its native audit architecture and x32 branch' : 'filter description failed', facts: { ...description, policy_sha256: policySha, helper_sha256: seccompHelperSha, raw_stdout_base64: describeResult.stdout.toString('base64') } }), stderr: Buffer.alloc(0) }, parserId: 'seccomp-description', parserRule: 'Reparse the raw helper stdout and require the current native AUDIT_ARCH, x32 kill branch on x86_64, and a nonempty BPF program.' });

  const dumpResult = await runProcess([helperPaths.seccomp, '--dump-bpf'], { timeoutMs: 10_000 });
  filterSha = sha256(dumpResult.stdout);
  const auditArch = process.arch === 'x64' ? 'AUDIT_ARCH_X86_64' : process.arch === 'arm64' ? 'AUDIT_ARCH_AARCH64' : 'unsupported';
  const filterRecord = addRecord({
    id: 'host.seccomp-filter', category: 'seccomp_filter', runtime: 'host', argv: [helperPaths.seccomp, '--dump-bpf'], result: dumpResult,
    parserId: 'seccomp-filter', parserRule: canonical({ expected_bpf_sha256: filterSha, policy_sha256: policySha, helper_sha256: seccompHelperSha, audit_arch: auditArch, x32_rejection: process.arch === 'x64' ? 'kill_process' : 'not_applicable' }),
  });
  filterAvailable = filterRecord.derived.availability === 'available' && description.audit_arch === auditArch && (process.arch !== 'x64' || description.x32_rejection === true);

  const rawResult = await runProcess([helperPaths.seccomp, '--raw-probe'], { timeoutMs: 10_000 });
  const rawRecord = addRecord({ id: 'host.seccomp-raw', category: 'seccomp_raw_probe', runtime: 'host', argv: [helperPaths.seccomp, '--raw-probe'], result: rawResult, parserId: 'seccomp-raw', parserRule: 'Require raw clone3=-1/ENOSYS with no child and raw legacy clone namespace flags denied with EPERM under the exact candidate filter.' });
  ...
  if (process.arch === 'x64') {
    ...
    addRecord({ id: 'host.seccomp-x32', category: 'seccomp_raw_probe', runtime: 'host', argv: [helperPaths.seccomp, '--probe-x32'], result: x32Result, parserId: 'x32-probe', parserRule: 'On x86_64, derive availability only from the child terminating with SIGSYS before syscall dispatch.' });
  } else {
    addRecord({ id: 'host.seccomp-x32', category: 'seccomp_raw_probe', runtime: 'host', argv: [helperPaths.seccomp, '--probe-x32'], result: syntheticResult(jsonBuffer({ availability: 'not_applicable', reason: 'x32 ABI exists only on x86_64', facts: { arch: process.arch } })), parserId: 'json-facts', parserRule: 'x32 rejection is not applicable off x86_64.' });
  }
}

const sentinelScript = path.join(dirs.pluginConfig, 'ordinary-child-wait.sh');
fs.writeFileSync(sentinelScript, String.raw`#!/bin/sh
set -eu
out=$1
pid_out=$2
(
...
async function runtimeSpawnRow(runtime) {
  ...
  if (Number.isInteger(childPid) && childPid > 1 && process.platform === 'linux') {
    try { procAfter = fs.readFileSync(`/proc/${childPid}/stat`, 'utf8'); } catch (error) { procAfter = error.code || String(error); }
    childReaped = procAfter === 'ENOENT';
  } else if (Number.isInteger(childPid) && childPid > 1) {
    try { process.kill(childPid, 0); childReaped = false; } catch (error) { childReaped = error.code === 'ESRCH'; procAfter = error.code || String(error); }
  }
  const envelope = {
    runtime_stdout_base64: runtimeResult.stdout.toString('base64'),
    runtime_stderr_base64: runtimeResult.stderr.toString('base64'),
...
function buildSummary() {
  ...
  const timing = records.filter((record) => record.category === 'attach_timing' && record.derived.availability === 'available');
  ...
  const common = [cgroup, namespace, filter, raw, x32].every((record) => record?.derived.availability === 'available' || record?.derived.availability === 'not_applicable');
  const runtimeSummary = {};
  for (const runtime of RUNTIMES) {
    const spawnRecord = records.find((record) => record.id === `spawn.${runtime}.candidate-filter`);
    const failed = [cgroup, namespace, filter, raw, x32, spawnRecord].filter((record) => !record || !['available', 'not_applicable'].includes(record.derived.availability)).map((record) => record?.id || 'missing_record');
    ...
    runtimeSummary[runtime] = {
      ...
      strong_cgroup_reason: strong ? 'all common strong-cgroup prerequisites and the real runtime child+wait probe passed' : `failed prerequisites: ${failed.join(',') || 'unknown'}`,
      filter_sha256: filterSha,
    };
  }
  return {
...
function validateShape(evidence) {
  const expectedTop = ['schema_version', 'schema_sha256', 'generated_at', 'harness', 'invocation', 'isolation', 'records', 'hash_chain', 'summary'];
  ...
  exactKeys(evidence.isolation, ['artifact_root', 'home', 'codex_home', 'claude_config_dir', 'plugin_config', 'relay_store', 'cwd', 'sentinels', 'mode'], 'isolation');
  exactKeys(evidence.hash_chain, ['algorithm', 'genesis', 'head', 'record_count'], 'hash_chain');
  exactKeys(evidence.summary, ['shared_protocol', 'protocol_tree', 'protocol_reason', 'protocol_lineage_deadline_ms', 'worker_tree_threat_model', 'managed_attach_deadline_ms', 'managed_attach_deadline_within_limit', 'runtimes'], 'summary');
  exactKeys(evidence.summary.runtimes, ['claude', 'codex'], 'summary.runtimes');
  for (const runtime of RUNTIMES) exactKeys(evidence.summary.runtimes[runtime], ['version', 'auth', 'hook_rows', 'strong_cgroup', 'strong_cgroup_reason', 'filter_sha256'], `summary.runtimes.${runtime}`);
  assert.equal(Array.isArray(evidence.records), true);
  assert.ok(evidence.records.length > 0);
  assert.equal(evidence.records.filter((record) => record.category === 'hook_contract').length, 12, 'exactly 12 hook contract rows');
  assert.equal(evidence.records.filter((record) => record.category === 'attach_timing').length, 20, 'exactly 10 timing rows per runtime');
  const ids = new Set();
  const recordKeys = ['id', 'category', 'runtime', 'runtime_version', 'platform', 'argv', 'started_at', 'finished_at', 'duration_ms', 'exit_status', 'signal', 'timed_out', 'stdout', 'stderr', 'parser', 'derived', 'previous_hash', 'record_hash'];
  ...
  for (const record of evidence.records) {
    ...
    assert.equal(ids.has(record.id), false, `duplicate evidence id ${record.id}`); ids.add(record.id);
    ...
    assert.equal(Array.isArray(record.argv) && record.argv.length > 0 && record.argv.every((arg) => typeof arg === 'string'), true, `${record.id} argv`);
    assert.equal(Number.isInteger(record.duration_ms) && record.duration_ms >= 0, true, `${record.id} duration`);
    assert.match(record.stdout.sha256, /^[0-9a-f]{64}$/); assert.match(record.stderr.sha256, /^[0-9a-f]{64}$/);
    assert.match(record.record_hash, /^[0-9a-f]{64}$/); assert.match(record.previous_hash, /^[0-9a-f]{64}$/);
    ...
    const parser = parsers.get(record.parser.id); assert.ok(parser, `${record.id} parser exists`);
    assert.deepEqual(parser(record, stdout, stderr), record.derived, `${record.id} derived evidence is parser-owned`);
    const withoutHashes = { ...record }; delete withoutHashes.previous_hash; delete withoutHashes.record_hash;
    assert.equal(sha256(`${record.previous_hash}\n${canonical(withoutHashes)}`), record.record_hash, `${record.id} hash`);
...
const localBlob = firstLine(spawnSync('git', ['hash-object', SCRIPT_REL], { cwd: ROOT, encoding: 'utf8' }).stdout || '');
...
const evidence = {
  ...
  generated_at: isoNow(),
  harness: { path: SCRIPT_REL, git_blob_oid: committedBlob, sha256: sha256(fs.readFileSync(path.join(ROOT, SCRIPT_REL))) },
  invocation: { argv: [process.execPath, SCRIPT_REL, '--verify-current'], real_runtime_test: true, verify_current: true },
  isolation: {
  ...
  records,
  hash_chain: { algorithm: 'sha256(previous_hash + LF + canonical_record_without_hashes)', genesis: GENESIS, head: records.at(-1).record_hash, record_count: records.length },
  summary,
};
...
function sealArtifacts(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) { sealArtifacts(full); fs.chmodSync(full, 0o500); }
...
console.log(`managed_attach_deadline_ms=${summary.managed_attach_deadline_ms === null ? 'unavailable' : summary.managed_attach_deadline_ms} within_20s=${summary.managed_attach_deadline_within_limit}`);
for (const runtime of RUNTIMES) {
  const row = summary.runtimes[runtime];
  if (row.strong_cgroup === 'available') console.log(`PASS strong_cgroup_spawn runtime=${runtime} clone3_errno=ENOSYS filter_sha256=${row.filter_sha256}`);
